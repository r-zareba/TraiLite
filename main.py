...
prices_manager = PostgresManager()
prices_manager.init(db_config)
...
def dax_update():
    ...
    if datetime.datetime.now().second == 0:
        if not dax_updating:
            ...
            prices_manager.insert_ohlc(asset='DAX', ohlc=ohlc)
...
def eurusd_update():
    ...
    if datetime.datetime.now().second == 0:
        if not eurusd_updating:
            ...
            prices_manager.insert_ohlc(asset='EURUSD', ohlc=ohlc)
            if not prices_printed:
                prices_printed = True
                tl.logger.info(f'{Color.UNDERLINE}{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}{Color.END} :')
...
def gbpusd_update():
    ...
    if datetime.datetime.now().second == 0:
        if not gbpusd_updating:
            ...
            prices_manager.insert_ohlc(asset='GBPUSD', ohlc=ohlc)
