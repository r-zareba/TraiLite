...
@tl.job(interval=dt.timedelta(milliseconds=PRICE_READ_INTERVAL))
def dax_update():
    global dax_updating
    global dax_position
    global dax_api
    ...
    if dt.datetime.now().second == 0:
        if not dax_updating:
            dax_updating = True
            ohlc = OHLC.from_prices_list(asset='DAX', prices_list=dax_prices_list)
            ohlc.save()
            if not prices_printed:
                prices_printed = True
                tl.logger.info(f'{Color.UNDERLINE}{dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}{Color.END} :')
...
def eurusd_update():
    ...
    if dt.datetime.now().second == 0:
        if not eurusd_updating:
            ...
            ohlc = OHLC.from_prices_list(asset='EURUSD', prices_list=eurusd_prices_list)
            ohlc.save()
            if not prices_printed:
                prices_printed = True
                tl.logger.info(f'{Color.UNDERLINE}{dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}{Color.END} :')
...
def gbpusd_update():
    ...
    if dt.datetime.now().second == 0:
        if not gbpusd_updating:
            ...
            ohlc = OHLC.from_prices_list(asset='GBPUSD', prices_list=gbpusd_prices_list)
            ohlc.save()
            if not prices_printed:
                prices_printed = True
                tl.logger.info(f'{Color.UNDERLINE}{dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}{Color.END} :')
...
# TODO
@tl.job(interval=dt.timedelta(minutes=5))
